...
export const AuthProvider = (props) => {
  ...
  const savetodrive = async (audioId) => {
    try {
      ...
      if (response.ok) {
        ...
        alert('Save to drive successful');
      } else if (response.status === 422) {
        const errorData = await response.json();
        console.error('Validation Error:', errorData);
      } else {
  ...
  const obrolangpt = async (accessToken, karakter, input) => {
    try {
      const response = await fetch(`${api}aiu/obrolan-gpt`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'access-token': accessToken,
  ...
  const obrolanbeta = async (karakter, input, bahasa) => {
    try {
      const accessToken = getCookie('access_token');
  
      const response = await fetch(`${api}aiu/obrolan-beta`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'access-token': accessToken,
  ...
  const smdlogin = async (username, password) => {
    try {
      const response = await fetch(`${api}auth/smd/login`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.error('Error during SMD login:', err.message);
      throw new Error('Error during SMD login');
    }
  };

  const smdregister = async (username, email, displayName, password) => {
    try {
      const response = await fetch(`${api}auth/smd/register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.error('Error during SMD registration:', err.message);
      throw new Error('Error during SMD registration');
    }
  };
