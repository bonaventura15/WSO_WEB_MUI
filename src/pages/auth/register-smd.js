...
const Page = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, helpers) => {
            const { username, email, displayName, password } = values;
            try {
                const res = await auth.smdregister(username, email, displayName, password);
                if (res != 'err') {
                    router.push(res);
                }
            } catch (err) {
            ...
            >
                <Box sx={{
                    ...
                    <div style={{ width: '750px' }}>
                        <Typography style={{ fontSize: '56px', fontFamily: 'inter', fontWeight: '600' }}>Connect To Sync, Manage, and Direct (SMD)</Typography>
                    </div>
                    <div style={{
                        width: '850px',
                        height: '400px',
