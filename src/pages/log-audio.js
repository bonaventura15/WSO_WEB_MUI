...
const Page = () => {
    ...
    const [dialogData, setDialogData] = useState({ translate: '', transcript: '' });
    ...
    const openSharePopup = () => {
        if (selectedAudioLog) {
            try {
                const translateData = selectedAudioLog.translate;
                const transcriptData = selectedAudioLog.transcript;
                ...
                console.error('Error setting dialog data:', error);
            }
        } else {
            console.error('No audio log selected');
        }
    };
    ...
    const handleDownloadAudio = async () => {
        try {
            ...
            const { audio_id } = selectedAudioLog;

            const audioData = await auth.getaudiodata(audio_id);
            console.log('Audio Data:', audioData);

            if (audioData && audioData.download_audio) {
                window.open(audioData.download_audio, '_blank');
            } else {
                console.error('Failed to retrieve audio data or download_audio URL is undefined');
            }
        } catch (error) {
            console.error('Error handling audio download:', error.message);
        }
    };
    ...
    const handleSavetoDrive = async () => {
        try {
            ...
            const { audio_id } = selectedAudioLog;
            await auth.savetodrive(audio_id);
    ...
    const handletosmd = async () => {
        try {
            ...
            await auth.sharetosmd(audio_id, shareCaption, accessToken);

            console.log('Shared to SMD successfully!');
        } catch (error) {
            console.error('Error sharing to SMD:', error.message);
        } finally {
            closeSharePopup();
        }
    };
                ...
                >
                    <List>
                        ...
                        <ListItem style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            <Button onClick={handleSavetoDrive} style={{ marginTop: '10px', flexDirection: 'column' }}>
                            ...
                            <Button onClick={openSharePopup} style={{ marginTop: '10px', flexDirection: 'column' }}>
                            ...
                            <Dialog
                                ...
                                PaperProps={{ style: { backgroundColor: '#404040', border:'8px solid rgba(0, 0, 0, 0.25)'} }}
                            >                               
                             <DialogTitle style={{ color: '#fff', fontFamily: 'Anonymous Pro', fontSize: '25px'}}>Post/Share To Sync Manage Direct (SMD)</DialogTitle>
                                <DialogContent>
                                    <TextField
                                        id="outlined-basic"
                                        label="Caption"
                                        ...
                                        />
                                    <div style={{width:'auto', height: '50px', border: '1px solid #FFF', borderRadius: '10px', marginTop: '10px'}}>
                                    <Typography style={{ marginLeft: '10px', marginTop: '13px', color: '#fff'}}>Translate: {dialogData.translate}</Typography>
                                    </div>
                                    <div style={{width:'auto', height: '50px', border: '1px solid #FFF', borderRadius: '10px', marginTop: '10px'}}>
                                    <Typography style={{ marginLeft: '10px', marginTop: '13px', color: '#fff'}}>Transcript: {dialogData.transcript}</Typography>
                                    </div>
                                </DialogContent>
                                <DialogActions style={{justifyContent: 'center'}}>
                                    <Button onClick={handletosmd} style={{border: '3px solid #FFF', color: '#fff', width: '130px', fontSize: '25px'}}>POST</Button>
                                </DialogActions>
                            </Dialog>
                        </ListItem>
                    </List>
